export interface Ingredient {
  id: string
  name: string
  category: string | null
  alcohol_content: number | null
  is_common: boolean
  image_url: string | null
}

export interface Category {
  id: string
  name: string
  icon: string | null
  color: string | null
}

export interface GlassType {
  id: string
  name: string
  emoji: string
}

export interface Technique {
  id: string
  name: string
  description: string | null
}

export interface DrinkIngredient {
  ingredient_id: string
  quantity: number
  unit: string
  is_optional: boolean
  sort_order: number
  notes: string | null
  ingredient: Ingredient
}

export interface Drink {
  id: string
  name: string
  description: string | null
  image_url: string | null
  instructions: string
  difficulty: 'easy' | 'medium' | 'hard'
  preparation_time_minutes: number
  alcohol_content: number | null
  category_id: string | null
  glass_type_id: string | null
  technique_id: string | null
  drink_ingredients: DrinkIngredient[]
  glass_type?: GlassType
  technique?: Technique
  category?: Category
  base_alcohol?: string
  flavor_profile?: string[]
  tags?: string[]
}

export interface DrinkMatch {
  drink: Drink
  score: number
  matchCount: number
  totalRequired: number
  missing: Ingredient[]
  canMake: boolean
}

export const CATEGORY_LABELS: Record<string, string> = {
  spirit:  'Destilados',
  liqueur: 'Licores',
  fresh:   'Frutas & Frescas',
  juice:   'Sucos',
  mixer:   'Mixers',
  syrup:   'Xaropes',
  other:   'Outros',
}

export const CATEGORY_ICONS: Record<string, string> = {
  spirit:  '🥃',
  liqueur: '🍾',
  fresh:   '🍋',
  juice:   '🧃',
  mixer:   '💧',
  syrup:   '🍯',
  other:   '✨',
}

export const DIFFICULTY_LABELS: Record<string, string> = {
  easy:   'Fácil',
  medium: 'Médio',
  hard:   'Difícil',
}

export const DIFFICULTY_COLORS: Record<string, string> = {
  easy:   '#3FB950',
  medium: '#D29922',
  hard:   '#F85149',
}

export const TECHNIQUE_EQUIPMENT: Record<string, string[]> = {
  'Shaker':  ['🍹 Coqueteleira', '🧊 Gelo', '🔍 Coador', '🥄 Colher de bar'],
  'Mexer':   ['🥃 Copo mixing (ou jarra)', '🥄 Colher de bar longa', '🧊 Gelo', '🔍 Coador'],
  'Montar':  ['🧊 Gelo', '🥄 Colher de bar'],
  'Bater':   ['🔨 Muddler (socador)', '🥄 Colher de bar', '🧊 Gelo'],
  'Blend':   ['🌀 Liquidificador', '🧊 Gelo'],
  'Flutuar': ['🥄 Colher de bar', '🧊 Gelo'],
  'Camadas': ['🥄 Colher de bar'],
}

export const TECHNIQUE_TIP: Record<string, string> = {
  'Shaker':  'Coloque os ingredientes e gelo na coqueteleira, feche bem e agite com força por 10-15 segundos até a coqueteleira ficar gelada por fora.',
  'Mexer':   'Use a colher longa para mexer suavemente em movimentos circulares. Não agite — o objetivo é resfriar sem aeração.',
  'Montar':  'Adicione os ingredientes diretamente no copo, na ordem indicada. Sem coqueteleira necessária.',
  'Bater':   'Use o muddler para macerar (amassar levemente) os ingredientes sólidos no fundo do copo antes de adicionar os líquidos.',
  'Blend':   'Bata todos os ingredientes com gelo no liquidificador até obter textura cremosa e homogênea.',
  'Flutuar': 'Despeje o ingrediente lentamente sobre o dorso de uma colher para que flutue sobre os outros sem misturar.',
  'Camadas': 'Adicione cada ingrediente devagar pelo dorso de uma colher, do mais denso para o mais leve.',
}

const DRINK_IMAGE_FILES: Record<string, string> = {
  // Brasileiros
  'Caipirinha':                 'caipirinha.jpg',
  'Caipiroska':                 'caipiroska.jpg',
  'Caipirinha de Morango':      'caipirinha-morango.jpg',
  'Caipirinha de Maracujá':     'caipirinha-maracuja.jpg',
  'Caipirinha de Melancia':     'caipirinha-melancia.jpg',
  'Caipirinha de Abacaxi':      'caipirinha-abacaxi.jpg',
  'Caipirinha de Uva':          'caipirinha-uva.jpg',
  'Caipiroska de Manga':        'caipiroska-manga.jpg',
  'Batida de Coco':             'batida-coco.jpg',
  'Batida de Maracujá':         'batida-maracuja.jpg',
  'Batida de Banana':           'batida-banana.jpg',
  'Batida de Abacaxi':          'batida-abacaxi.jpg',
  'Rabo de Galo':               'rabo-de-galo.jpg',
  'Caju Amigo':                 'caju-amigo.jpg',
  'Macunaíma':                  'macunaima.jpg',
  'Quentão':                    'quentao.jpg',
  'Leite de Onça':              'leite-de-onca.jpg',
  'Stinger':                    'stinger.jpg',
  // Clássicos
  'Mojito':                     'mojito.jpg',
  'Virgin Mojito':              'virgin-mojito.jpg',
  'Margarita':                  'margarita.jpg',
  'Margarita Frozen':           'margarita-frozen.jpg',
  "Tommy's Margarita":          'tommy-margarita.jpg',
  'Watermelon Margarita':       'watermelon-margarita.jpg',
  'White Margarita':            'white-margarita.jpg',
  'Spicy Margarita':            'spicy-margarita.jpg',
  'Gin Tônica':                 'gin-tonica.jpg',
  'Vodka Tônica':               'vodka-tonica.jpg',
  'Cosmopolitan':               'cosmopolitan.jpg',
  'Negroni':                    'negroni.jpg',
  'Mezcal Negroni':             'mezcal-negroni.jpg',
  'Daiquiri':                   'daiquiri.jpg',
  'Strawberry Daiquiri':        'strawberry-daiquiri.jpg',
  'Peach Daiquiri':             'peach-daiquiri.jpg',
  'Moscow Mule':                'moscow-mule.jpg',
  'Mango Mule':                 'mango-mule.jpg',
  'Strawberry Mule':            'strawberry-mule.jpg',
  'Apple Mule':                 'apple-mule.jpg',
  'Cassis Mule':                'cassis-mule.jpg',
  'Aperol Spritz':              'aperol-spritz.jpg',
  'Hugo Spritz':                'hugo-spritz.jpg',
  'Spritz Veneziano':           'spritz-veneziano.jpg',
  'Cassis Spritz':              'cassis-spritz.jpg',
  'Piña Colada':                'pina-colada.jpg',
  'Virgin Piña Colada':         'virgin-pina-colada.jpg',
  'Old Fashioned':              'old-fashioned.jpg',
  'Martini Clássico':           'martini-classico.jpg',
  'Vesper Martini':             'vesper-martini.jpg',
  'Pornstar Martini':           'pornstar-martini.jpg',
  'Espresso Martini':           'espresso-martini.jpg',
  'Pear Martini':               'pear-martini.jpg',
  'Chocolate Martini':          'chocolate-martini.jpg',
  'Tequila Sunrise':            'tequila-sunrise.jpg',
  'Sex on the Beach':           'sex-on-the-beach.jpg',
  'Whisky Sour':                'whisky-sour.jpg',
  'Rum Sour':                   'rum-sour.jpg',
  'Amaretto Sour':              'amaretto-sour.jpg',
  'Pisco Sour':                 'pisco-sour.jpg',
  'Midori Sour':                'midori-sour.jpg',
  'Sake Sour':                  'sake-sour.jpg',
  'Calvados Sour':              'calvados-sour.jpg',
  'Cherry Sour':                'cherry-sour.jpg',
  'Vanilla Vodka Sour':         'vanilla-vodka-sour.jpg',
  'Bloody Mary':                'bloody-mary.jpg',
  'Virgin Mary':                'virgin-mary.jpg',
  'Caesar':                     'caesar.jpg',
  'Cuba Libre':                 'cuba-libre.jpg',
  'Dark and Stormy':            'dark-and-stormy.jpg',
  'Long Island Iced Tea':       'long-island-iced-tea.jpg',
  'Long Island Iced Tea Clássico': 'long-island-iced-tea.jpg',
  'White Russian':              'white-russian.jpg',
  'Black Russian':              'black-russian.jpg',
  'Alexander':                  'alexander.jpg',
  'Manhattan':                  'manhattan.jpg',
  'Rob Roy':                    'rob-roy.jpg',
  'Boulevardier':               'boulevardier.jpg',
  'French 75':                  'french-75.jpg',
  'Sidecar':                    'sidecar.jpg',
  'Between the Sheets':         'between-the-sheets.jpg',
  'Sazerac':                    'sazerac.jpg',
  'Last Word':                  'last-word.jpg',
  'Gimlet':                     'gimlet.jpg',
  'Tom Collins':                'tom-collins.jpg',
  'Gin Fizz':                   'gin-fizz.jpg',
  'Silver Fizz':                'silver-fizz.jpg',
  'New Orleans Fizz':           'gin-fizz.jpg',
  'Clover Club':                'clover-club.jpg',
  'Aviation':                   'aviation.jpg',
  'Bramble':                    'bramble.jpg',
  'Harvey Wallbanger':          'harvey-wallbanger.jpg',
  'Screwdriver':                'screwdriver.jpg',
  'Vodka Cranberry':            'vodka-cranberry.jpg',
  'Paloma':                     'paloma.jpg',
  'Penicillin':                 'penicillin.jpg',
  'Paper Plane':                'paper-plane.jpg',
  'Naked and Famous':           'naked-and-famous.jpg',
  'Rusty Nail':                 'rusty-nail.jpg',
  'Kir':                        'kir.jpg',
  'Kir Royal':                  'kir-royal.jpg',
  // Tropicais
  'Blue Lagoon':                'blue-lagoon.jpg',
  'Zombie':                     'zombie.jpg',
  'Mai Tai':                    'mai-tai.jpg',
  'Jungle Bird':                'jungle-bird.jpg',
  'Pain Killer':                'pain-killer.jpg',
  'Passion Cup':                'passion-cup.jpg',
  'Tahiti Club':                'tahiti-club.jpg',
  'Gauguin':                    'gauguin.jpg',
  'Chi-Chi':                    'chi-chi.jpg',
  'Acapulco':                   'acapulco.jpg',
  'Beachcomber':                'beachcomber.jpg',
  'Cardinal':                   'cardinal.jpg',
  'Malibu Sunrise':             'malibu-sunrise.jpg',
  'Malibu Cola':                'malibu-cola.jpg',
  'Coco Loco':                  'coco-loco.jpg',
  'Tropical Gold':              'tropical-gold.jpg',
  'Orange Punch':               'orange-punch.jpg',
  'Papaia Tropical':            'papaia-tropical.jpg',
  'Abacaxi Fizz':               'abacaxi-fizz.jpg',
  'Vodka Melancia':             'vodka-melancia.jpg',
  'Limoncello Spritz':          'limoncello-spritz.jpg',
  // Cremosos
  'Grasshopper':                'grasshopper.jpg',
  'Mudslide':                   'mudslide.jpg',
  'Alexandra':                  'alexandra.jpg',
  'Banshee':                    'banshee.jpg',
  'Blow Job':                   'blow-job.jpg',
  'Frostbite':                  'frostbite.jpg',
  'La Jolla':                   'la-jolla.jpg',
  'Nutcracker':                 'nutcracker.jpg',
  'Orgasmo':                    'orgasmo.jpg',
  'Scooter':                    'scooter.jpg',
  'Silk Stockings':             'silk-stockings.jpg',
  'Olé':                        'ole.jpg',
  'Esquimo':                    'esquimo.jpg',
  'Banana Cream':               'banana-cream.jpg',
  'Affogato Cocktail':          'affogato-cocktail.jpg',
  'Eggnog':                     'eggnog.jpg',
  'Tom and Jerry':              'tom-and-jerry.jpg',
  'Porto Flip':                 'porto-flip.jpg',
  // Shots
  'Tequila Shot':               'tequila-shot.jpg',
  'B52':                        'b52.jpg',
  'Kamikaze':                   'kamikaze.jpg',
  'Tequila Boom':               'tequila-boom.jpg',
  'Shot de Baileys':            'shot-baileys.jpg',
  'Lemon Drop Shot':            'lemon-drop-shot.jpg',
  'La Bomba':                   'la-bomba.jpg',
  'Earthquake':                 'earthquake.jpg',
  'Prairie Fire':               'prairie-fire.jpg',
  'T.G.V.':                     't-g-v.jpg',
  'Alabama Slammer':            'alabama-slammer.jpg',
  'Anisette Shot':              'anisette-shot.jpg',
  // Sem álcool
  'Shirley Temple':             'shirley-temple.jpg',
  'Sunrise Mocktail':           'sunrise-mocktail.jpg',
  'Limonada Suíça':             'limonada-suica.jpg',
  'Mocktail Tropical':          'mocktail-tropical.jpg',
  'Arnold Palmer':              'arnold-palmer.jpg',
  'Apple Cobbler':              'apple-cobbler.jpg',
  'Chantaco':                   'chantaco.jpg',
  'Florida':                    'florida-mocktail.jpg',
  'Honeymoon':                  'honeymoon.jpg',
  'Lemon Mint':                 'lemon-mint.jpg',
  "Mermaid's Song":             'mermaids-song.jpg',
  'Paradis Punch':              'paradis-punch.jpg',
  'Spring Fever':               'spring-fever.jpg',
  // Agridoces extras
  'Sangria':                    'sangria.jpg',
  'Mimosa':                     'mimosa.jpg',
  'Bellini':                    'bellini.jpg',
  'Framboise Royale':           'framboise-royale.jpg',
  'Espumante com Morango':      'espumante-morango.jpg',
  'Porto Tônico':               'porto-tonico.jpg',
  'Hot Toddy':                  'hot-toddy.jpg',
  'Michelada':                  'michelada.jpg',
  'Shandy':                     'shandy.jpg',
  'Peach Fizz':                 'peach-fizz.jpg',
  'Strawberry Gin Fizz':        'strawberry-gin-fizz.jpg',
  'Raspberry Cosmo':            'raspberry-cosmo.jpg',
  'Mint Frappé':                'mint-frappe.jpg',
  'Macieira Cola':              'macieira-cola.jpg',
  // Clássicos do PDF
  'Godmother':                  'godmother.jpg',
  'Alexiana':                   'alexiana.jpg',
  'Allelúia':                   'alleluia.jpg',
  'Big John':                   'big-john.jpg',
  'Black Lady':                 'black-lady.jpg',
  "Devil's Tail":               'devil-tail.jpg',
  'East India':                 'east-india.jpg',
  'Flamingo':                   'flamingo.jpg',
  'Golden Dawn':                'golden-dawn.jpg',
  'Green Hope':                 'green-hope.jpg',
  'Hammerhead':                 'hammerhead.jpg',
  'Hot Pants':                  'hot-pants.jpg',
  'Lady Killer':                'lady-killer.jpg',
  'Purple Passion':             'purple-passion.jpg',
  'Quaker':                     'quaker.jpg',
  'Shady Lady':                 'shady-lady.jpg',
  'Sneaky Pete':                'sneaky-pete.jpg',
  'T-Bird':                     't-bird.jpg',
  'Woman Warrior':              'woman-warrior.jpg',
  'Crimson':                    'crimson.jpg',
  'Loch Lomond':                'loch-lomond.jpg',
  // Outros
}

function getFallbackByName(drink: Drink): string {
  const n = drink.name.toLowerCase()
  if (n.includes('caipir') || n.includes('batida')) return '/drinks/caipirinha.jpg'
  if (n.includes('mojito')) return '/drinks/mojito.jpg'
  if (n.includes('margarita')) return '/drinks/margarita.jpg'
  if (n.includes('mule')) return '/drinks/moscow-mule.jpg'
  if (n.includes('sour')) return '/drinks/whisky-sour.jpg'
  if (n.includes('martini')) return '/drinks/martini-classico.jpg'
  if (n.includes('spritz')) return '/drinks/aperol-spritz.jpg'
  if (n.includes('daiquiri')) return '/drinks/daiquiri.jpg'
  if (n.includes('colada')) return '/drinks/pina-colada.jpg'
  if (n.includes('shot') || n.includes('b52') || n.includes('kamikaze')) return '/drinks/tequila-shot.jpg'
  if (n.includes('virgin') || n.includes('mocktail')) return '/drinks/virgin-mojito.jpg'
  if (n.includes('russian')) return '/drinks/white-russian.jpg'
  if (n.includes('negroni')) return '/drinks/negroni.jpg'
  if (n.includes('gin')) return '/drinks/gin-tonica.jpg'
  if (n.includes('vodka')) return '/drinks/cosmopolitan.jpg'
  if (n.includes('rum') || n.includes('cuba')) return '/drinks/cuba-libre.jpg'
  if (n.includes('whisky') || n.includes('whiskey') || n.includes('bourbon')) return '/drinks/old-fashioned.jpg'
  if (n.includes('tequila')) return '/drinks/margarita.jpg'
  if (n.includes('champagne') || n.includes('mimosa') || n.includes('bellini')) return '/drinks/mimosa.jpg'
  if (n.includes('sangria')) return '/drinks/sangria.jpg'
  if (n.includes('cerveja') || n.includes('shandy') || n.includes('michelada')) return '/drinks/michelada.jpg'
  if (n.includes('porto')) return '/drinks/porto-tonico.jpg'
  if (n.includes('sake')) return '/drinks/sake-sour.jpg'
  if (n.includes('mezcal')) return '/drinks/mezcal-negroni.jpg'
  if (n.includes('cremoso') || n.includes('cream')) return '/drinks/white-russian.jpg'
  return '/drinks/placeholder.jpg'
}

export function getDrinkImage(drink: Drink): string {
  if (drink.image_url) return drink.image_url
  const file = DRINK_IMAGE_FILES[drink.name]
  if (file) return `/drinks/${file}`
  return getFallbackByName(drink)
}